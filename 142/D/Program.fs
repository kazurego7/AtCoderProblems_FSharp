...
module NumericFunctions =
    ...
    let primeFactrization (n : int64) : seq<int64 * int64> =
        ...
        | _ ->
            let ps = primes (int32 (sqrt (float n))) |> Seq.toArray |> InputOutputs.int64s |> Array.toSeq
            if Seq.forall (fun p -> n % p <> 0L) ps then seq { yield (n, 1L) }
            else
                // a * b^x からxを求める
                let rec invPow a b x =
                    if (a % b <> 0L) then x
                    else invPow (a / b) b (x + 1L)
                ps
                    |> Seq.filter (fun p -> n % p = 0L)
                    |> Seq.map (fun p -> (p, invPow n p 0L))
...
let main _ =
    let [| A; B |] = readInt64s()
    let cd = commonDivisor A B |> Seq.sort |> Seq.toArray
    let len = Seq.length cd
    let ans =
        if len = 1 then len
        else
            let mutable ps = [ cd.[1] ]
            for cdi in Array.skip 2 cd do
                if List.forall (fun p -> cdi % p <> 0L) ps then ps <- cdi :: ps
            Seq.length ps + 1
