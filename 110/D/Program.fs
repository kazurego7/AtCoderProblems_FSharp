...
let main _ =
    let [| N; M |] = readInt32s()
    let mods = { divisor = 1000000007 }
    let pf =
        if M = 1 then seq { yield (1L, 0L) }
        else primeFactrization (int64 M)
        |> Seq.map (fun (p, count) -> (int32 p, int32 count))
    let ans =
        pf
        |> Seq.map (fun (_, count) ->
            mods.Div (mods.Perm (count + N - 1) count) (mods.Perm count count))
        |> Seq.fold mods.Mul 1
    print ans
