...
let main _ =
    ...
    let dscIndex =
        Seq.interval 0 M
        |> Seq.sortByDescending (fun i -> C.[i])
        |> Seq.toArray

    let cumB =
        dscIndex
        |> Array.scan (fun state i -> state + int64 B.[i]) 0L
    
    let right =
        leftBinarySearch (int64 (M + 1)) (fun i -> cumB.[int32 i] <= int64 N)
        |> Option.map int32
    ...
    let dscC =
        ...
        | Some r ->
            let front =
                Seq.interval 0 r
                |> Seq.collect (fun i ->
                    Seq.replicate B.[dscIndex.[i]] C.[dscIndex.[i]])
            let back =
                if r = M then
                    Seq.replicate (N - int32 cumB.[r]) 0
                else
                    Seq.replicate (N - int32 cumB.[r]) C.[dscIndex.[r]]
            Seq.append front back
        |> Seq.toArray
    let ans =
        Seq.interval 0 N
        |> Seq.sumBy (fun i -> max dscC.[i] ascA.[i] |> int64)
