...
let main _ =
    let N = readInt32 ()
    let L = readInt32s ()
    let sortedL = Array.sort L
    let mutable count = 0
    for i in Seq.interval 0 (N - 2) do
        let a = sortedL.[i]
        for j in Seq.interval (i + 1) (N - 1) do
            let b = sortedL.[j]
            let rec loop k isBreak res =
                if k = N || isBreak then
                    res
                else
                    let c = sortedL.[k]
                    if (a < b + c && b < c + a && c < a + b) then
                        loop (k + 1) false (res + 1)
                    else
                        loop (k + 1) true res
            count <- loop (j + 1) false count
