...
[<EntryPoint>]
let main _ =
    let [|N;M;L|] = readInt32s ()
    let ABC = readMatrixInt32 M
    let A = ABC.[*, 0]
    let B = ABC.[*,1]
    let C = ABC.[*,2]
    let Q = readInt32 ()
    let st = readMatrixInt32 Q
    let s = st.[*,0]
    let t = st.[*,1]

    let matrix =
        Array2D.zeroCreate N N
        |> Array2D.map (fun x -> None)
    for i in Seq.interval 0 M do
        matrix.[A.[i], B.[i]] <- Some C.[i]
        matrix.[B.[i], A.[i]] <- Some C.[i]
    
    let shortestPath =
        Array2D.zeroCreate N N
        |> Array2D.map (fun x -> None)
    
    for b in Seq.interval 0 N do
        for a in Seq.interval 0 N do
            for c in Seq.interval 0 N do
                match matrix.[a,b], matrix.[b,c] with
                | Some ab, Some bc ->
                    let ac = shortestPath.[a,c]
                    if (ac < ab + bc)
                    shortestPath.[a,c] <- 
                | _ -> unit
