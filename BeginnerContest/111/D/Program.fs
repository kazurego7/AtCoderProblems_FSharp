...
let main _ =
    let N = readInt32()
    let XY = readMatrix N |> Array2D.map int32
    let X = XY.[*, 0]
    let Y = XY.[*, 1]
    let M = 31
    ...
    let Us = Seq.map2 (+) X Y
    let Vs = Seq.map2 (-) X Y
    if not ((Seq.forall (int64 >> isEven) Us)
        || (Seq.forall (int64 >> isOdd) Us))
    then
        print -1
    else
        let ds =
        ...
        let getRot u v =
            let mutable rot = ""
            let uIsEven = isEven (int64 u)
            let mutable x = (0, 0)
            for i in Seq.interval 0 M |> Seq.rev do
                match x with
                | (u1, v1) when u < u1 && v >= v1 ->
                    rot <- "D" + rot
                    x <- (u1 - ds.[i], v1 + ds.[i])
                | (u1, v1) when u < u1 && v < v1 ->
                    rot <- "L" + rot
            ...
            if uIsEven then
                ...
                | (u1, v1) when u >= u1 && v >= v1 ->
                    rot <- "R" + rot
            rot
        if Seq.forall (int64 >> isOdd) Us then
            print M
            printRow ds
            printColumn (Seq.map2 getRot Us Vs)
        else
            print (M + 1)
            printRow (seq { yield 1; yield! ds })
            printColumn (Seq.map2 getRot Us Vs)
    0
