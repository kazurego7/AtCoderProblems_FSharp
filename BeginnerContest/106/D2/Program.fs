...
[<EntryPoint>]
let main _ =
    let [| N; M; Q |] = readInt32s()
    let LR = readMatrixInt32 M
    let L = LR.[*, 0]
    let R = LR.[*, 1]
    let pq = readMatrixInt32 Q
    let p = pq.[*, 0]
    let q = pq.[*, 1]

    let x = Array2D.zeroCreate (N + 1) (N + 1)
    for i in Seq.interval 0 M do
        x.[L.[i], R.[i]] <- x.[L.[i], R.[i]] + 1

    let cum = Array2D.zeroCreate (N + 1) (N + 1)
    for i in Seq.interval 1 (N + 1) do
        for k in Seq.interval 1 (N + 1) do
            cum.[i, k] <- x.[i, k] + cum.[i - 1, k] + cum.[i, k - 1] - cum.[i - 1, k - 1]

    let cumSquare (l: int32) (r: int32): int32 = cum.[r, r] - cum.[l - 1, r] - cum.[r, l - 1] + cum.[l - 1, l - 1]
    let ans = Seq.interval 0 Q |> Seq.map (fun i -> cumSquare p.[i] q.[i])
