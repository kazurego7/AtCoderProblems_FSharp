...
let main _ =
    let L = read ()
    let N = L.Length
    let dp = Array2D.zeroCreate (N + 1) 2
    ...
    for i in Seq.interval 0 N do
        ...
        if L.[i] = '0' then
            ...
            dp.[i+1,1] <- mods.Add dp.[i,0] dp.[i,1]
        
        // (a, b) = (0, 1) (1, 0)
        if L.[i] = '0' then
            dp.[i+1,1] <- mods.Add dp.[i+1,1] (mods.Mul dp.[i,1] 2)
        else
            dp.[i+1,0] <- mods.Add dp.[i+1,0] (mods.Mul dp.[i,0] 2)
            dp.[i+1,1] <- mods.Add dp.[i+1,1] (mods.Mul dp.[i,1] 2)
    
    let ans = mods.Add dp.[N,0] dp.[N,1]
    print ans
