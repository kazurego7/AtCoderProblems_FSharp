...
let rec f n = if (BigInteger.Compare(BigInteger 2, n) = 1) then BigInteger.One else n * (f (n - BigInteger 2))

[<EntryPoint>]
let main _ =
    let strN = read ()
    let ans =
        if isOdd (int64 strN) then
            0
        else
            let n10 =
                strN
                |> Seq.rev
                |> Seq.mapi (fun i strNi ->
                    let Ni = int32 (string strNi)
                    Ni * pown 2 i)
                |> Seq.sum
            n10
    print (f (BigInteger (int64 strN)))
    print ans
