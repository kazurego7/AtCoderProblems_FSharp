...
let main _ =
    ...
    let cumXor = Array.scan (fun state Ai -> state ^^^ Ai) 0L A
    let cumSum = Array.scan (fun state Ai -> state + Ai) 0L A

    let indexes = Seq.interval 0 N |> Seq.toArray
    let ans =
        Seq.interval 0 N
        |> Seq.sumBy (fun i ->
            let pred k =
                let xor = cumXor.[k + 1] ^^^ cumXor.[i]
                let sum = cumSum.[k + 1] - cumSum.[i]
                xor = sum
            let bound = binarySearch indexes pred N i
            int64 (bound - i + 1))
    print ans
