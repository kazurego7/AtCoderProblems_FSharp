...
let main _ =
    let [|N;C|] = readInt32s()
    let D = readMatrixInt32 C
    let c = readMatrixInt32 N
    ...
    let sumDGrid =
        ...
        |> Seq.map (fun r ->
            ...
            |> Seq.map (fun color ->
                let sumD =
                    Seq.interval 0 N |> Seq.sumBy (fun i ->
                        Seq.interval 0 N |> Seq.sumBy (fun k ->
                            if (i + k) % 3 = r then
                                D.[c.[i, k]-1, color]
                            else 
                                0))
                (color, sumD))
            |> Seq.sortBy (fun (color, sumD) -> sumD)
            |> Seq.take 3
            |> Seq.toArray
        )
        |> Seq.toArray
    let ans =
        Seq.interval 0 3 |> Seq.collect (fun i ->
            Seq.interval 0 3 |> Seq.collect (fun j ->
                Seq.interval 0 3 |> Seq.choose (fun k ->
                    let [(c1 ,d1); (c2, d2); (c3, d3)] =
                        [yield sumDGrid.[0].[i]; yield sumDGrid.[1].[j]; yield sumDGrid.[2].[k]]
