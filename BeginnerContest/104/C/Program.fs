...
let main _ =
    ...
    let ans =
        Seq.interval 0 (pown 2 D)
        |> Seq.map (fun flag ->
            let completeScore =
                Seq.interval 0 D
                |> Seq.filter (checkFlag flag)
                |> Seq.sumBy (fun i -> c.[i] + (i + 1) * 100 * p.[i])
            let completeNum =
                Seq.interval 0 D
                |> Seq.filter (checkFlag flag)
                |> Seq.sumBy (fun i -> p.[i])
            let basicScores =
                Seq.interval 0 D
                |> Seq.rev
                |> Seq.filter (checkFlag flag >> not)
                |> Seq.collect (fun i -> Seq.replicate (p.[i] - 1) ((i+1)*100) )
                |> Seq.scan (+) 0
            let basicNum =
                basicScores
                |> Seq.takeWhile (fun basicScore -> basicScore + completeScore < G)
                |> Seq.length
            if Seq.last basicScores + completeScore >= G then
                completeNum + basicNum
            else 
                inf
            )
        |> Seq.min
