...
let main _ =
    ...
    for i in Seq.interval 0 (N - 1) do
        tree.[a.[i] - 1] <- (b.[i] - 1, i) :: tree.[a.[i] - 1]
        tree.[b.[i] - 1] <- (a.[i] - 1, i) :: tree.[b.[i] - 1]

    let colors = Array.create (N - 1) 0
    let reached = Array.create N false

    let mutable maxCount = 0

    let rec dfs (parent: int32) (parentColor: int32): unit =
        reached.[parent] <- true
        maxCount <- max maxCount (List.length tree.[parent])
        let mutable color = 1
        for child, index in tree.[parent] do
            if not reached.[child] then
                if color = parentColor then color <- color + 1
                colors.[index] <- color
                dfs child color
                color <- color + 1
