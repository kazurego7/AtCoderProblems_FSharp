...
let main _ =
    ...
    let edgeCounts = Array.create N 0
    let edgeColors = new Dictionary<int32 * int32, int32>()
    let parentEdgeColor = Array.create N None
    ...
    while not (Seq.isEmpty order) do
        ...
        // ************ノードの処理***************
        let edgeCount = List.length tree.[node]
        ...
        // ************エッジの処理***************
        let colors =
            match parentEdgeColor.[node] with
            | None -> [ 1 .. edgeCount ]
            | Some parentColor ->
                if parentColor > (edgeCount - 1) then [ 1 .. edgeCount - 1 ]
                else List.append [ 1 .. parentColor - 1 ] [ parentColor + 1 .. edgeCount ]
        for child, color in List.zip childs colors do
            edgeColors.Add((node, child), color)
            edgeColors.Add((child, node), color)
            parentEdgeColor.[child] <- Some color
        // **************************************
        for child in childs do
            order.Enqueue(child)
            reached.[child] <- true

    print (Array.max edgeCounts)
    let ans = Seq.interval 0 (N - 1) |> Seq.map (fun i -> edgeColors.Item(a.[i] - 1, b.[i] - 1))
    printColumn ans
