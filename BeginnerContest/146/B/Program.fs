...
let main _ =
    ...
    let src = [ 'A' .. 'Z' ] |> List.map string
    let dst = List.append (List.skip N src) (List.take N src)
    let dict = Map.ofList (List.zip src dst)

    let ans =
        S
        |> Seq.map (fun Si -> Map.find (string Si) dict)
        |> String.concat ""
    print ans
