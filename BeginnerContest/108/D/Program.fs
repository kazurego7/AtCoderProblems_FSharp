...
let main _ =
    let L = readInt32()
    let N = (int32 (Math.Log(float L, 2.0))) + 1
    let twices0 =
        Seq.interval 0 (N - 1)
        |> Seq.map (fun i -> string (i + 1) + " " + string (i + 2) + " " + string 0)
    let twices1 =
        Seq.interval 0 (N - 1)
        |> Seq.map (fun i -> string (i + 1) + " " + string (i + 2) + " " + string (1 <<< (N - 2 - i)))
    let plusOnes =
        Seq.interval 0 (N - 1)
        |> Seq.rev
        |> Seq.choose
            (fun i ->
            if ((L >>> (N - 2 - i)) &&& 1) = 1 then
                string 1 + " " + string (i + 2) + " " + string (((L >>> (N - 2 - i)) - 1) <<< (N - 2 - i))
                |> Some
            else
                None
            )
    let ans = Seq.append (Seq.append twices0 twices1) plusOnes

    print (string N + " " + string (Seq.length ans))
    printColumn ans
