...
let main _ =
    let [|N;Q|] = readInt32s ()
    let STX = readMatrixInt32 N
    let S = STX.[*,0]
    let T = STX.[*,1]
    let X = STX.[*,2]
    let mutable D = (readMatrixInt32 Q).[*,0]

    let sortedIx =
        Seq.interval 0 N
        |> Seq.sortBy (fun i -> X.[i])
    let ans =
        Array.zeroCreate<int32> Q
        |> Array.map (fun i -> -1)

    for k in sortedIx do
        let l = 
            rightBinarySearch (int64 D.Length) (fun i -> S.[k] - X.[k] <= D.[int32 i])
            |> Option.map int32
        let r =
            leftBinarySearch (int64 D.Length) (fun i -> D.[int32 i] < T.[k] - X.[k])
            |> Option.map int32
        match (l, r) with
        | Some left, Some right when left <= right ->
            printfn "%A %A" left right
            printRow D
            for e in seq {left .. right} do
                ans.[e] <- X.[k]
            let mutable nextD = Set D
            for e in seq {left .. right} do
                nextD <- Set.remove D.[e] nextD
            D <- (Set.toArray nextD)
        | _ -> ()
