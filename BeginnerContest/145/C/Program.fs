...
let rec go (px, py) rests : float =
    match rests with
    | [||] -> 0.0
    | _ ->
        Seq.interval 0 (Array.length rests)
        |> Seq.sumBy (fun i ->
            let nx,ny = rests.[i]
            let nextRests = Array.append (Array.take i rests) (Array.skip (i + 1) rests)
            let d = sqrt (float (pown (nx - px) 2 + pown (ny - py) 2))
            d + go (nx, ny) nextRests )
        

[<EntryPoint>]
let main _ =
    let N = readInt32 ();
    let xy = readMatrixInt32 N
    let x = xy.[*,0]
    let y = xy.[*,1]
    let poss =
        Array.zip x y
    let denom = float (Seq.fold (*) 1 (seq {1..N}))
    let numer =
        Seq.interval 0 N
        |> Seq.sumBy (fun i -> 
            go poss.[i] (Array.append (Array.take i poss) (Array.skip (i + 1) poss)))
