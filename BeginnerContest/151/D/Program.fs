...
let main _ =
    ...
    let uf = UnionFind.UnionFind(H * W)

    for h in Seq.interval 0 H do
        for w in Seq.interval 0 W do
            // left
            if w - 1 >= 0 && S.[h, w - 1] = "." then
                uf.Unite (W * h + w) (W * h + w - 1)
            // up
            if h - 1 >= 0 && S.[h - 1, w] = "." then
                uf.Unite (W * h + w) (W * (h - 1) + w)
            // right
            if w + 1 < W && S.[h, w + 1] = "." then
                uf.Unite (W * h + w) (W * h + w + 1)
            // down
            if h + 1 < H && S.[h + 1, w] = "." then
                uf.Unite (W * h + w) (W * (h + 1) + w)
    
    let groupS = Array.create (H * W) []
    for h in Seq.interval 0 H do
        for w in Seq.interval 0 W do
            let id = uf.Id (W * h + w)
            groupS.[id] <- (h, w) :: groupS.[id]
    
    let ans = 
        groupS
        |> Seq.map (fun routes ->
            let G = Array2D.create (H*W) (H*W) (-1)
            for (h1,w1) in routes do
                for (h2, w2) in routes do
                    if h1 = h2 && w1 = w2 then
                        G.[h1*W + w1,h2*W + w2] <- 0
            ...
            for h in Seq.interval 0 (H*W) do
                printRow G.[h, *]
