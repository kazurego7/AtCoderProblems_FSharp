...
let main _ =
    ...
    let inf = pown 2 28
    let G = Array4D.create H W H W inf



    for yi in Seq.interval 0 H do
        for xi in Seq.interval 0 W do
            for yj in Seq.interval 0 H do
                for xj in Seq.interval 0 W do
                    if (yi = yj && xi = xj) then G.[yi, xi, yj, xj] <- 0
                    if (yi = yj && xi - 1 = xj || yi - 1 = yj && xi = xj || yi = yj && xi + 1 = xj
                        || yi + 1 = yj && xi = xj) && (S.[yi, xi] = "." && S.[yj, xj] = ".") then
                        G.[yi, xi, yj, xj] <- 1

    for yk in Seq.interval 0 H do
        for xk in Seq.interval 0 W do
            for yi in Seq.interval 0 H do
                for xi in Seq.interval 0 W do
                    for yj in Seq.interval 0 H do
                        for xj in Seq.interval 0 W do
                            G.[yi, xi, yj, xj] <- min G.[yi, xi, yj, xj] (G.[yi, xi, yk, xk] + G.[yk, xk, yj, xj])
    ...
    for yi in Seq.interval 0 H do
        for xi in Seq.interval 0 W do
            for yj in Seq.interval 0 H do
                for xj in Seq.interval 0 W do
                    if G.[yi, xi, yj, xj] <> inf then
                        ans <- max ans G.[yi, xi, yj, xj]
    print ans
