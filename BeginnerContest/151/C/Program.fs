...
let main _ =
    let [|N;M|] = readInt32s ()
    let pS = readMatrix M
    let p = pS.[*, 0] |> Array.map int
    let S = pS.[*, 1]

    let probs = Array.create N []
    for i in Seq.interval 0 M do
        probs.[p.[i] - 1] <- S.[i] :: probs.[p.[i] - 1]
        
    let wa =
        Seq.interval 0 N
        |> Seq.sumBy (fun i ->
            let ps = List.rev probs.[i] 
            if List.exists (fun x -> x = "AC") ps then
                List.findIndex (fun x -> x = "AC") ps
            else 
                0)
    
    let ac =
        Seq.interval 0 N
        |> Seq.filter (fun i -> List.exists (fun x -> x = "AC") (List.rev probs.[i]))
        |> Seq.length
    
    printRow [yield ac; yield wa]
