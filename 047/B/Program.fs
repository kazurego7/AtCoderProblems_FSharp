...
[<EntryPoint>]
let main _ =
    let [| W; H; N |] = readInt32s()
    let xya = readMatrix N
    let x = xya.[*, 0] |> int32s
    let y = xya.[*, 1] |> int32s
    let a = xya.[*, 2] |> int32s
    ...
    let ais ai =
        Seq.interval 0 N
            |> Seq.filter (fun i -> a.[i] = ai)

    let left =
        ais 1
            |> Seq.map (fun i -> x.[i])
            |> Seq.append (seq { yield 0 })
            |> Seq.max

    let right =
        ais 2
            |> Seq.map (fun i -> x.[i])
            |> Seq.append (seq { yield W })
            |> Seq.min
    ...
    let up =
        ais 4
            |> Seq.map (fun i -> y.[i])
            |> Seq.append (seq { yield H })
            |> Seq.min


    let ans = (max 0 (right - left)) * (max 0 (up - down))
